...
.hero{text-align:center;padding:3rem 1rem 1.5rem;
  background:radial-gradient(ellipse 80% 60% at 50% -10%,#6c63ff33,transparent);}
...
.badge{display:inline-block;margin-bottom:.8rem;
  background:linear-gradient(135deg,#6c63ff22,#00d4ff22);
  border:1px solid #6c63ff55;border-radius:50px;
  padding:.2rem .9rem;font-size:.72rem;color:var(--accent2);
  letter-spacing:2px;text-transform:uppercase;font-weight:600;}

/* ── Card columns — target Streamlit's column wrapper ── */
[data-testid="column"]{
...
[data-testid="column"]:hover{
  ...
  box-shadow:0 0 24px #6c63ff14 !important;
}
...
[data-testid="stRadio"] div[role="radiogroup"] > label:has(input:checked) {
  border-color: var(--accent) !important;
  background: #6c63ff1e !important;
  box-shadow: 0 0 12px #6c63ff28 !important;
  color: #fff !important;
}
...
.warn-banner{background:#ffb34715;border:1px solid #ffb34740;border-radius:10px;
  padding:.75rem 1rem;color:#ffb347;font-weight:500;font-size:.85rem;margin:.5rem 0;}
</style>
...
st.markdown("""
<div class="hero">
  <div class="badge">⚡ SheetFusion</div>
...
with c1:
    ...
    st.markdown('<div class="card-title">📂 Upload First Spreadsheet</div>', unsafe_allow_html=True)
...
with c2:
    ...
    st.markdown('<div class="card-title">📂 Upload Second Spreadsheet</div>', unsafe_allow_html=True)
...
if file1 and file2:
    ...
    with s1:
        ...
        st.markdown('<div class="card-title">🗂 File 1 — Sheet Selection</div>', unsafe_allow_html=True)
    ...
    with s2:
        ...
        st.markdown('<div class="card-title">🗂 File 2 — Sheet Selection</div>', unsafe_allow_html=True)
    ...
    if df1 is not None and df2 is not None:
        ...
        <div style="text-align:center;margin-bottom:1.2rem">
          ...
          <div style="font-size:1.25rem;font-weight:700;margin-top:.3rem">
        ...
        with p1:
            ...
            st.markdown('<div class="card-title">🔑 Pick merge key — File 1</div>', unsafe_allow_html=True)
            sel1 = st.radio("key1", df1.columns.tolist(), index=None,
                            key="col1_radio", label_visibility="collapsed")
            if sel1:
                st.markdown(f'<div class="ok-banner">✓ Selected: <strong>{sel1}</strong></div>',
                            unsafe_allow_html=True)
        with p2:
            st.markdown('<div class="step-lbl">File 2 columns</div>', unsafe_allow_html=True)
            st.markdown('<div class="card-title">🔑 Pick merge key — File 2</div>', unsafe_allow_html=True)
            sel2 = st.radio("key2", df2.columns.tolist(), index=None,
                            key="col2_radio", label_visibility="collapsed")
            if sel2:
                st.markdown(f'<div class="ok-banner">✓ Selected: <strong>{sel2}</strong></div>',
                            unsafe_allow_html=True)
        ...
        if sel1 and sel2:
            ...
            with mc:
                ...
                merge_type = st.selectbox(
                    ...
                    format_func=lambda x: {
                        ...
                        "left":  "⬅️  Left  — Keep all rows from File 1 only",
                        "right": "➡️  Right — Keep all rows from File 2 only",
                ...
                <div style="background:#6c63ff12;border:1px solid #6c63ff33;border-radius:10px;
                  ...
                  <strong style="color:#a89cff">⚙️ Duplicate Match Handling</strong><br>
                ...
                dup_mode = st.selectbox(
                    ...
                    index=0,
                    help="Sequential Match pairs duplicate values in order (1st to 1st, 2nd to 2nd) preventing row inflation without discarding entries. First Match Only drops subsequent duplicates. Many-to-Many merges every match combination.",
                    key="dup_mode_sel"
                )
            ...
            if do_merge:
                ...
                try:
                    d1 = df1.copy()
                    d2 = df2.copy()
                    # try numeric coerce on key columns for consistent matching
                    ...
                    if dup_mode == "First Match Only":
                        ...
                        merged = pd.merge(d1, d2,
                                          left_on=sel1, right_on=sel2,
                                          how=merge_type,
                                          suffixes=("_File1","_File2"))
                    elif dup_mode == "One-to-One (Sequential Match)":
                        # Create occurrence counters to match sequentially
                    ...
                    # ── Fix column order: File 1 cols LEFT, File 2 cols RIGHT ──
                    f1_cols = [c for c in merged.columns
                               if c in df1.columns or c.endswith("_File1")]
        ...
        if st.session_state.merged_df is not None:
            ...
            # For OUTER join, having more rows than either source is EXPECTED
            # (unmatched rows from both sides are appended). Only warn for
            # left/right/inner where the anchor file size should be preserved.
            ...
            if _show_warn:
                ...
                <div style="background:#ffb34710;border:1px solid #ffb34740;border-radius:12px;
                  ...
                  <div style="color:#ffb347;font-weight:700;font-size:.92rem;margin-bottom:.4rem">
                    ⚠️ Row count inflated — Many-to-Many matches detected
                  </div>
                  <div style="color:#c9a66b;font-size:.82rem;line-height:1.65">
                    ...
                    <strong>Fix:</strong> Set the <em>"Duplicate handling strategy"</em> dropdown above to <strong>One-to-One (Sequential Match)</strong> or <strong>First Match Only</strong>.
                  </div>
                </div>""", unsafe_allow_html=True)
                ...
                <div style="background:#00d4ff10;border:1px solid #00d4ff40;border-radius:12px;
                  ...
                  <div style="color:#00d4ff;font-weight:700;font-size:.92rem;margin-bottom:.4rem">
                    ℹ️ Outer Join Explanation — Row count breakdown
                  </div>
                  <div style="color:#9bc9d3;font-size:.82rem;line-height:1.65">
                    ...
                    This is because you chose an <strong>Outer Join</strong> (Union), which is mathematically expected to be larger than either source file.<br>
                    <strong>Calculation:</strong> Unique File 1 Keys + Unique File 2 Keys - Overlapping Keys.<br>
                    Since unmatched rows from both files are kept, the total row count represents the combined unique set.<br>
                    💡 <em>If you want the final merged file to keep exactly the rows of File 1 (without appending unmatched File 2 records at the bottom), switch the strategy above to <strong>Left Join</strong>.</em>
                  </div>
                </div>""", unsafe_allow_html=True)

            # Stats
            matched = int((merged[sel1 if merge_type != "right" else sel2] != "").sum()) if sel1 and sel2 else 0
            ...
            st.markdown('<div class="card-title">🔎 Merged Preview (first 100 rows)</div>',
                        unsafe_allow_html=True)

            # skeleton before table
            sk2 = st.empty()
...
<div style="text-align:center;margin-top:4rem;padding-top:2rem;
  border-top:1px solid #1e2640;color:#6b7280;font-size:.8rem">
